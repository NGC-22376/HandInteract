...
class GestureRecognitionModel(nn.Module):
    def __init__(self, input_size, num_classes, max_len):
        ...
        # 计算全连接层的输入维度
        self.fc_input_dim = self._compute_fc_input_dim(max_len)

        # 全连接层
        self.fc1 = nn.Linear(self.fc_input_dim, 64)
        self.fc2 = nn.Linear(64, num_classes)

    def _compute_fc_input_dim(self, max_len):
        """ 计算全连接层的输入大小 """
        with torch.no_grad():  # 不计算梯度，避免浪费显存
            x = torch.randn(1, 1, max_len)  # 形状 (batch_size=1, channels=1, sequence_length=max_len)
            x = torch.relu(self.conv1(x))
            x = self.pool1(x)
            x = torch.relu(self.conv2(x))
            x = self.pool2(x)
            return x.view(1, -1).shape[1]  # 展平后返回特征数量

    def forward(self, x):
        x = x.unsqueeze(1)  # 添加 channel 维度，使输入变为 (batch_size, 1, seq_length)
        x = torch.relu(self.conv1(x))
        x = self.pool1(x)
        x = torch.relu(self.conv2(x))
        x = self.pool2(x)

        x = x.view(x.size(0), -1)  # 展平为 (batch_size, 特征数量)
...
# 模型实例化（不需要考虑GPU设备）
num_classes = len(gestures)  # 手势类别数
model = GestureRecognitionModel(input_size=1, num_classes=num_classes, max_len=max_len)
