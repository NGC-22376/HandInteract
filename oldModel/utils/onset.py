...
dataset_path = r"D:\download\feishu_download\dataset_3.3"#按3.5秒长度截取后的数据文件夹，理论上每个文件中存放一个混有静息数据的动作数据
output_dir = r"D:\download\feishu_download\fragment3"#空文件夹，用于存放有效动作的数据
outputplt_dir = r"D:\download\feishu_download\fragment3可视化文件"#空文件夹，用于存放图的空文件夹
MAJOR_LW = 2.5
MINOR_LW = 1.5
MAX_ROWS = 10
...
def data_processing(dataset_path, is_cwt,sampling_rate,show):
    ...
    for idx, category in enumerate(categories):
        ...
        for file_name in os.listdir(path):
            ...
            origin_signal1 = df.iloc[:, 1].values.astype(np.float32)
            origin_signal2 = df.iloc[:, 1].values.astype(np.float32)
            data_raw = torch.tensor(origin_signal1, dtype=torch.float32)
            data_raw2 = torch.tensor(origin_signal2, dtype=torch.float32)
            filtered_signal = signal_filter(data_raw2)  # 滤波
            # 归一化
            ...
            normalized_data_rest = normalized_data_filter[:20000]
            # 添加到列表内
            ...
            threshold = var_rest * 0.25 + np.mean(x_rest[:])
            ...
            onsets2, tflist, smooth = bonato_onset_detector(signal=x_filter, rest=x_rest, sampling_rate=sampling_rate,
                                                            threshold=threshold, active_state_duration=1000,
                                                            samples_above_fail=100, fail_size=500)
            ...
            fragment = x_filter[onsets2[0]:onsets2[1]]

            # 生成输出路径
            os.makedirs(os.path.join(output_dir, category), exist_ok=True)

            output_path = os.path.join(output_dir, category,f'4.{m}.0.csv')
            # 保存片段
            ...
            outputplt_path = os.path.join(outputplt_dir, category, file_name)
            # plot
            '''
            onsets_all = []
...
def bonato_onset_detector(signal=None, rest=None, sampling_rate=1000.,
    ...
    onset = False
    alarm = False
    for k in range(1, len(mvgav2), 2):  # odd values only
        # calculate the test function
        tf = (1 / var_rest) * (mvgav2[k-1]**2 + mvgav2[k]**2)
        tf_list.append(tf)
        if onset is True:
            if alarm is False:
                if tf < threshold:
    ...
    if len(offset_time_list)>0:
        ...
        onsets = np.union1d(onset_time_list[0],int(len(mvgav2)/2)-1)

    # adjust indices because of odd numbers
    onsets *= 2

    return ReturnTuple((onsets, tf_list,mvgav2), ('onsets', 'processed','smooth'))


show=1
data_processing(dataset_path,0,25000.0,show)
